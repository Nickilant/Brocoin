...
def get_user(request):
    ...
    if not user:
        refs = json.dumps({'id': []})
        cursor.execute(f"INSERT INTO public.users (sid,username,score,last_score,last_tap,ref_code,refs,energy,tickets,mining_claim, last_login, reward_streak) VALUES ('{uuid.uuid4()}','{username}',25,0,'{datetime.now()}','{user_id}',{repr(refs)},1000,25, True, '{date.today()}',1)")
        ...
        if ref_code:
            ...
            if not premium:
                cursor.execute(f"UPDATE users set refs='{json.dumps(actual_refs)}', tickets = {int(actual_tickets) + 10} where ref_code='{ref_code}'")
            else:
                cursor.execute(
                    f"UPDATE users set refs='{json.dumps(actual_refs)}', tickets = {int(actual_tickets) + 50}, score = {int(actual_score) + 50} where ref_code='{ref_code}'")
    cursor.execute(f"SELECT * FROM users where username = '{username}'")
    user = cursor.fetchall()
